<div class="container">
    ...
    if ($res->num_rows > 0){
        while ($row = $res->fetch_assoc()){
            ...
            $html .= '<td class="text-center"><a class="action" href="?action=delete&id=' . $row ['id'] . '" title="usuń" onclick="return confirm(\'Czy na pewno chcesz usunąć ten wpis?\');"><i class="fa fa-times text-danger"></i>usuń</a>&nbsp;
            <a class="action" href="?action=edit&id=' . $row ['id'] . '" title="edytuj"><i class="fa fa-pencil text-warning"></i>edytuj</a></td>';
            $html .= '</tr>';
        }
        $html .= "</tbody>";
        $html .= "</table>";
